...
func TestMain(m *testing.M) {
	...
	if err != nil {
		log.Fatal().Msgf("Could not connect to docker: %v", err)
	}
...
func truncateAll(t *testing.T, ctx context.Context) {
	_, err := db.Exec(ctx, `DO $$ DECLARE
										  	r RECORD;
											BEGIN
											  FOR r IN (SELECT tablename FROM pg_tables WHERE schemaname ='shared_mining') LOOP
												EXECUTE 'TRUNCATE TABLE contracts.' || quote_ident(r.tablename) || ' CASCADE';
											END LOOP;
										  END $$;`)
	require.NoError(t, err)

	_, err = db.Exec(ctx, `DROP SCHEMA shared_mining CASCADE`)
	require.NoError(t, err)
}
