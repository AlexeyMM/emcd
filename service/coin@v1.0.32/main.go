...
func main() {
	...
	err = app.New().
		...
		Run(ctx)
	if err != nil {
		log.Error(ctx, "coin service shutdown: %s", err.Error())
	}
...
type Deps struct {
	...
	GRPCClient struct {
		...
		Node       node.NodeServiceClient
		Rate       ratePb.RateClient
	}
	Repository struct {
...
func NewDeps(ctx context.Context, cfg config.Config) (Deps, error) {
	deps := Deps{Cfg: cfg}
	if cfg.FeeMultiplierErc20 <= 0 {
		return deps, fmt.Errorf("unexpected FeeMultiplierErc20: %v", cfg.FeeMultiplierErc20)
	}
...
func (d *Deps) makeGRPCClient(cfg config.Config) error {
	...
	wlClient, err := grpc.NewClient(cfg.WhiteLabelAddress, dialOptions...)
	if err != nil {
		return fmt.Errorf("dial white lable client: %w", err)
	}
	d.GRPCClient.WhiteLabel = whitelabel.NewWhitelabelServiceClient(wlClient)

	nodeClient, err := grpc.NewClient(cfg.NodeAddress, dialOptions...)
	if err != nil {
		return fmt.Errorf("dial node client: %w", err)
	}

	d.GRPCClient.Node = node.NewNodeServiceClient(nodeClient)

	rateClient, err := grpc.NewClient(cfg.RateAddress, dialOptions...)
	if err != nil {
		return fmt.Errorf("dial rate client: %w", err)
	}

	d.GRPCClient.Rate = ratePb.NewRateClient(rateClient)
