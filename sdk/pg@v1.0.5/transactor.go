...
type PgxTransactor interface {
	...
	WithinTransactionWithOptions(ctx context.Context, txFn func(ctx context.Context) error, opts pgx.TxOptions) error
	Runner(ctx context.Context) PgxQueryRunner
}

type pgxTransactor struct {
	pool   *pgxpool.Pool
	runner PgxWithinTransactionRunner
}

// NewPgxTransactor builds new PgxTransactor
func NewPgxTransactor(p *pgxpool.Pool) PgxTransactor {
	return &pgxTransactor{pool: p, runner: NewPgxWithinTransactionRunner(p)}
}

// WithinTransaction runs WithinTransactionWithOptions with default tx options
func (t *pgxTransactor) WithinTransaction(ctx context.Context, txFunc func(ctx context.Context) error) error {
...
func (t *pgxTransactor) WithinTransactionWithOptions(ctx context.Context, txFunc func(ctx context.Context) error, opts pgx.TxOptions) (err error) {
...
// Runner embedded pgx runner
func (t *pgxTransactor) Runner(ctx context.Context) PgxQueryRunner {
	return t.runner.Runner(ctx)
}
