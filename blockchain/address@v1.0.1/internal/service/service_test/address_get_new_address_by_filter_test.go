...
func TestAddressService_GetNewAddressesByFilter(t *testing.T) {
	...
	t.Run("success new get", func(t *testing.T) {
		...
		_, resp, err := s.GetNewAddressesByFilter(ctx, addressFilter)
	...
	t.Run("success processing get", func(t *testing.T) {
		addressFilter := &model.AddressFilter{
			Id:           &addressUuid,
			Address:      nil,
			UserUuid:     nil,
